...
volatile unsigned int one_second_flag=1; // Used for calculating the frequency
...
{
	// Timer 2 is used for holding the frequency value of the colpitts oscillator
	T3CON = 0x0; // Ensure timer is off and reset before changing settings
	PR3 = 0xffff; // When TMR2 hits 0xffff resets back to zero
	TMR3 = 0; // start the timer at zero
	T3CONbits.TCKPS = 0; // Pre-scaler: 1
	T3CONbits.TCS = 1; // external clock source
...
int main(void)
{
    ...
    unsigned short crc;
    unsigned int T3_overflow;
    unsigned int Base_freq,current_freq;
	DDPCON = 0;
	CFGCON = 0;
	INTCONbits.MVEC = 1;
    ...
    SetupTimer3();
 	...
 	/* Pin 14 (RB5 is used for the clock/counter input to timer3 */	
	
	ANSELB &= ~(1<<5); // Set RB5 as a digital I/O
	TRISB |= (1<<5);   // configure pin RB5 as input
	CNPUB |= (1<<5);   // Enable pull-up resistor for RB5
	T3CKRbits.T3CKR= 1; // The pin that is used as the input for the external clock is pin 14
	
	
	ANSELB &= ~(1<<3); // Set RB5 as a digital I/O
	TRISB |= (1<<3);   // configure pin RB5 as input
	CNPUB |= (1<<3);   // Enable pull-up resistor for RB5
	...
	Base_freq = Calibrate_Frequency();
	/*
	// Configuration Button Presses
    while (!CALIB_BUTTON);
	while (CALIB_BUTTON);
	Start_Playback(0x030cea, 0x0FA8A);
	while (play_flag);
	// After this button is pressed the actual configuration is done.
	
    while (!CALIB_BUTTON);
    while (CALIB_BUTTON);
	Start_Playback(0x040774, 0xFA8A);
	while(play_flag);
	////////////////////////////////// calibration, we take the 10th second
	for(k=0;k<5; k++){
	T3CONbits.ON = 0; // Turn off timer2
		TMR3 = 0; // Reset the timer count
		one_second_flag = 0; // Reset the 1 second flag counter
		T3_overflow = 0; // Reset the overflow counter
		T3CONbits.ON = 1; // Start the timer
		while(!one_second_flag) // while one second has not passed check for overflow in timer 2 counter
		{
			if(IFS0&_IFS0_T3IF_MASK)
			{
			IFS0CLR=_IFS0_T3IF_MASK; // Clear overflow flag
			T3_overflow++; // Increment overflow counter
			}
		}
		T3CONbits.ON = 0; // Turn the timer off 
		
		/* Here we take the value of the timer which is the frequency from the colpitts oscillator and based
		   on the frequency we output a specific voice indicating the type of metal
		*/
	/*	};
		Base_freq = (T3_overflow*0x100000+TMR3)-37;
		
	
		
	printf("base_freq =  %d Hz\r\n",Base_freq);
	...
	while(1)
	{
		...
		/* Here we take the value of the timer which is the frequency from the colpitts oscillator and based
		   on the frequency we output a specific voice indicating the type of metal
		*/
		current_freq =  (T3_overflow*0x100000+TMR3);
    ...
    if(play_flag == 0)
		{
			if(current_freq >= Base_freq+20 && current_freq <= Base_freq+300)
			{
			// This is a small ferrous material. Play that part of the index file
			
			Start_Playback(0x02380c,0xD4DE);
			}
			else if(current_freq >= Base_freq+500)
			{
			// This is a large ferrous material. Play that part of the index file
			
			Start_Playback(0x00914d,0xDDE6);
			}
			else if(current_freq <= Base_freq-5 && current_freq >= Base_freq-20)
			{
			// This is a small non-ferrous material. Play that part of the index file
			Start_Playback(0x016f33,0xC8D9);
			
			}
			else if(current_freq <= Base_freq-20)
			{
			// This is a large non-ferrous material. Play that part of the index file
			Start_Playback(0x00002b,0x914d);
			}
		}
 }
    return 1;
...
int Calibrate_Frequency(void) {

	unsigned int Base_freq;
	unsigned int T3_overflow;
	unsigned int k;

	Start_Playback(0x006cc1, 0x012ddb);
	while (!CALIB_BUTTON & play_flag);
	// After this button is pressed the actual configuration is done.
	
    while (!CALIB_BUTTON);
    while (CALIB_BUTTON);
	Start_Playback(0x02ebdd, 0x12131);
	...
	Base_freq = (T3_overflow*0x100000+TMR3);
	
	// Dividing by 9 to find the average of the nine base_freq samples
	// We also have some fudging factor so our initial "current_freqeuncy" matches our base frequency
